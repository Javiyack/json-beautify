import { calcDepth, byteSize } from './analyzer';
...
export function parseJson(input: string): ParseResult {
  ...
  try {
    ...
    const { line, column } = locateJsonError(input, err.message);
    const snippet = extractSnippet(input, line, column);
    const metrics = {
      parseTimeMs: performance.now() - start,
      depth: 0,
      sizeBytes: byteSize(input)
    };
    const error: ParseError = { message: err.message, line, column, snippet };
